    ...
    <div style="margin-top: 10px; margin-bottom: 80px">
        <el-card>
            <div style="padding: 20px; color: #888">
                <div>
                    <el-input v-model="entity.content" :rows="3" type="textarea"></el-input>
            ...
            <div v-for="item in messages" style="display: flex; padding: 20px">
                ...
                <div style="padding: 0 10px; flex: 5">
                    ...
                    <div style="padding: 10px 0; color: #888">
                        {{ item.content }}
                        <el-button v-if="item.username === user.username" size="mini" type="text" @click="del(item.id)">
                            删除
                        </el-button>
                    </div>
                    <div v-if="item.parentMessage" style="background-color: #eee; padding: 10px">{{
                            item.username
                        }}：{{ item.parentMessage.content }}
                    </div>
                    <div style="color: #888; font-size: 12px">
                        <span>{{ item.time }}</span>
                        <el-button style="margin-left: 20px" type="text" @click="reReply(item.id)">回复</el-button>
                    </div>
                </div>
            </div>

            <el-dialog v-model="dialogFormVisible" title="回复信息" width="30%">
                <el-form :model="entity" label-width="80px">
                    <el-form-item label="内容">
                        <el-input v-model="entity.reply" :rows="3" autocomplete="off" type="textarea"></el-input>
                    </el-form-item>
                </el-form>
                <template #footer>
